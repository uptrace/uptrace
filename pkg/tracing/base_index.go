...
type BaseIndexer[T indexer] struct {
	Type T
}

func NewBaseIndexer[T indexer](indexer T) *BaseIndexer[T] {
	return &BaseIndexer[T]{Type: indexer}
}
...
func (b *BaseIndexer[T]) initIndex(span *Span) {
	index := b.Type.GetBaseIndex()
	index.Span = span
