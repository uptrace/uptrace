...
var timers = sync.Pool{New: func() any { t := time.NewTimer(time.Hour); t.Stop(); return t }}
...
func (p *ConnPool) CloseConn(cn *Conn) error { p.removeConnAcquiringLock(cn); return p.closeConn(cn) }
...
func (p *ConnPool) Len() int { p.connsMu.Lock(); n := len(p.conns); p.connsMu.Unlock(); return n }
...
func withLock(mu *sync.Mutex, fn func()) { mu.Lock(); defer mu.Unlock(); fn() }
