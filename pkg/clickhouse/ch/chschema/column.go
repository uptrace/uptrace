...
func (c *ColumnOf[T]) SetValue(ptr unsafe.Pointer) {
	column := *(*[]T)(ptr)
	c.Column = column
}
...
func (c *baseJSONColumn) Init(chType string, goType reflect.Type) error {
	c.typ = goType
	return nil
}
...
func (c *MsgpackColumn) Init(chType string, goType reflect.Type) error {
	c.typ = goType
	return nil
}
func (c *MsgpackColumn) Clear() {
	clear(c.values)
	c.BytesColumn.Clear()
}
