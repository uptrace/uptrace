...
func (c *LCStringColumn) WritePrefix(wr *chproto.Writer) error { wr.Int64(lcVersion); return nil }
...
func newLCKeyType(typ int64) lcKey {
	switch typ {
	case 0:
		return lcKey{typ: 0, read: func(rd *chproto.Reader) (int, error) { n, err := rd.UInt8(); return int(n), err }, write: func(wr *chproto.Writer, n int) { wr.UInt8(uint8(n)) }}
	case 1:
		return lcKey{typ: int8(1), read: func(rd *chproto.Reader) (int, error) { n, err := rd.UInt16(); return int(n), err }, write: func(wr *chproto.Writer, n int) { wr.UInt16(uint16(n)) }}
	case 2:
		return lcKey{typ: 2, read: func(rd *chproto.Reader) (int, error) { n, err := rd.UInt32(); return int(n), err }, write: func(wr *chproto.Writer, n int) { wr.UInt32(uint32(n)) }}
	case 3:
		return lcKey{typ: 3, read: func(rd *chproto.Reader) (int, error) { n, err := rd.UInt64(); return int(n), err }, write: func(wr *chproto.Writer, n int) { wr.UInt64(uint64(n)) }}
	default:
		panic("not reached")
	}
}
