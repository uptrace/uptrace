...
import (
	"github.com/uptrace/pkg/clickhouse/ch/internal/parser"
...
var NopFmter Formatter

func FormatQuery(query string, args ...any) string { return NopFmter.FormatQuery(query, args...) }
func AppendQuery(b []byte, query string, args ...any) []byte {
	return NopFmter.AppendQuery(b, query, args...)
}

type Formatter struct{ args *namedArgList }

func NewFormatter() Formatter { return Formatter{} }
func (f Formatter) WithArg(arg NamedArgAppender) Formatter {
	return Formatter{args: f.args.WithArg(arg)}
}
func (f Formatter) WithNamedArg(name string, value any) Formatter {
	return Formatter{args: f.args.WithArg(&namedArg{name: name, value: value})}
}
func (f Formatter) FormatQuery(query string, args ...any) string {
	if (args == nil && f.args == nil) || strings.IndexByte(query, '?') == -1 {
		return query
	}
	return unsafeconv.String(f.AppendQuery(nil, query, args...))
}
...
func (l *namedArgList) WithArg(arg NamedArgAppender) *namedArgList {
	return &namedArgList{arg: arg, next: l}
}
...
func newStructArgs(fmter Formatter, strct any) (*structArgs, bool) {
	...
	return &structArgs{table: TableForType(v.Type()), strct: v}, true
}
