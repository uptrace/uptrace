...
func (q *InsertQuery) DistTable(table string) *InsertQuery {
	q.addTable(q.distTable(table))
	return q
}
...
func (q *InsertQuery) From(selq *SelectQuery) *InsertQuery {
	q.selq = selq
	return q
}
func (q *InsertQuery) Block(block *Block) *InsertQuery {
	q.block = block
	return q
}
