...
func NewCreateTableQuery(db *DB) *CreateTableQuery {
	return &CreateTableQuery{baseQuery: baseQuery{db: db, conn: db}}
}
func (q *CreateTableQuery) Model(model any) *CreateTableQuery { q.setTableModel(model); return q }
...
func (q *CreateTableQuery) Replace() *CreateTableQuery     { q.replace = true; return q }
...
func (q *CreateTableQuery) Operation() string { return "CREATE TABLE" }
...
func (q *CreateTableQuery) AppendQuery(fmter chschema.Formatter, b []byte) (_ []byte, err error) {
	...
	} else if q.table.Engine != "" {
		b = append(b, q.table.Engine...)
	} else {
...
func (q *CreateTableQuery) appendPartition(fmter chschema.Formatter, b []byte) ([]byte, error) {
	if q.partition.IsZero() && (q.table == nil || q.table.Partition == "") {
		return b, nil
	}
	...
	return append(b, q.table.Partition...), nil
}
